...
class UserService:
    ...
    async def user_login(self, user_data: UserLogin, db: AsyncSession):
        ...
        user = await self.authenticate_user(username=user_data.username, password=user_data.password, db=db)
        if not user:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Incorrect password"
        ...
        async with httpx.AsyncClient() as client:
            try:
                token = await client.post(f"{self.auth_service_url}/auth/token", json=user_data)
